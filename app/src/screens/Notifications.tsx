...
const meta: Record<string, { icon: string; tone: string }> = {
  ...
  report: { icon: '📊', tone: 'bg-slate-100 text-slate-600' },
  system: { icon: '🔔', tone: 'bg-slate-100 text-slate-600' },
}
  ...
  return (
    <div className="min-h-[100svh] bg-slate-50 pb-4">
      ...
      {groups.map((g) => {
        ...
        return (
          <div key={g}>
            <div className="px-5 pt-4 pb-1 text-[12px] font-bold text-slate-400 tracking-wide">{g.toUpperCase()}</div>
              ...
              {list.map((n) => {
                ...
                return (
                  <div key={n.id} className="flex items-start gap-3 px-4 py-3.5 border-b border-slate-100 last:border-0">
                    ...
                    <div className="flex-1 min-w-0"><b className="text-slate-800 text-sm">{n.title}</b><div className="text-[13px] text-slate-500 leading-snug">{n.body}</div></div>
                    <span className="text-[11px] text-slate-400 shrink-0">{new Date(n.createdAt).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })}</span>
                  </div>
                )
              })}
            </div>
