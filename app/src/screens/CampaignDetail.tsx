...
export default function CampaignDetail() {
  ...
  if (loading) return <div className="min-h-[100svh] app-canvas"><TopBar title="Chi tiết chiến dịch" back="/campaigns" /><Loading /></div>
  if (error || !c) return <div className="min-h-[100svh] app-canvas"><TopBar title="Chi tiết chiến dịch" back="/campaigns" /><ErrorBox message={error ?? 'Không tìm thấy'} /></div>
  ...
  return (
    <div className="min-h-[100svh] app-canvas">
      ...
      <div className={`relative h-44 bg-gradient-to-br ${c.gradient} grid place-items-center overflow-hidden`}>
        <div className="absolute -right-8 -top-10 w-40 h-40 rounded-full bg-white/15" />
        <div className="absolute -left-10 -bottom-12 w-44 h-44 rounded-full bg-black/10" />
        <div className="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent" />
        <span className="text-7xl drop-shadow-lg relative">{c.emoji}</span>
      </div>
      ...
      <div className="px-5 pt-4">
        ...
        <h1 className="text-xl font-extrabold text-ink-900 mt-2 leading-snug">{c.title}</h1>
        <div className="text-ink-400 text-sm mt-1">📍 {c.location}</div>
      </div>

      <div className="mx-5 mt-4 rounded-3xl p-5 text-white bg-gradient-to-br from-brand-600 to-brand-800 shadow-float">
      ...
      <div className="flex justify-around mt-4 px-5 text-[11px] text-ink-500 text-center">
        <div className="flex flex-col items-center gap-1"><BadgeCheck className="w-5 h-5 text-emerald-500" />Đã xác thực</div>
        <div className="flex flex-col items-center gap-1"><Clock className="w-5 h-5 text-brand-500" />Sổ quỹ công khai</div>
        <div className="flex flex-col items-center gap-1"><ShieldCheck className="w-5 h-5 text-emerald-500" />Chi phí có chứng từ</div>
      </div>

      <div className="mx-5 mt-4 grid grid-cols-3 bg-white/70 ring-1 ring-slate-900/5 rounded-2xl p-1 shadow-soft">
        {([['story', 'Câu chuyện'], ['donate', 'Quyên góp'], ['cost', 'Chi phí']] as [Tab, string][]).map(([k, l]) => (
          <button key={k} onClick={() => setTab(k)} className={`py-2 rounded-xl text-[13px] font-bold transition-all ${tab === k ? 'bg-white text-brand-700 shadow-card' : 'text-ink-500'}`}>{l}</button>
        ))}
      </div>
      ...
      <div className="px-5 mt-4 pb-6">
        {tab === 'story' && (
          <>
            <p className="text-ink-600 text-sm leading-relaxed">{c.story}</p>
            <h3 className="font-extrabold text-ink-900 mt-5 mb-3">Lộ trình triển khai</h3>
            <div className="space-y-1">
              {c.roadmap.map((r, i) => (
                <div key={i} className="flex gap-3 pb-4 relative">
                  ...
                  <div>
                    <div className="font-bold text-ink-900 text-sm">{r.label}</div>
                    <div className="text-xs text-ink-400">{r.date} · {r.desc}</div>
                  </div>
                </div>
              ))}
            </div>
            <h3 className="font-extrabold text-ink-900 mt-3 mb-3">Hình ảnh thực tế</h3>
            {c.photos.length > 0 ? (
              ...
              <div className="grid grid-cols-2 gap-2">
                {[0, 1].map((i) => (
                  <div key={i} className="h-24 rounded-2xl bg-slate-100 ring-1 ring-slate-900/5 grid place-items-center text-slate-300"><ImageIcon className="w-7 h-7" /></div>
                ))}
              </div>
            )}
          </>
        ...
        {tab === 'donate' && (
          <div className="space-y-2">
            {c.topDonors.length === 0 && <p className="text-ink-400 text-sm text-center py-6">Chưa có dữ liệu quyên góp.</p>}
            {c.topDonors.map((d, i) => (
              <div key={i} className="flex items-center gap-3 bg-white rounded-2xl p-3 ring-1 ring-slate-900/5 shadow-soft">
                ...
                <div className="flex-1 text-sm"><b>{d.anon ? 'Nhà hảo tâm ẩn danh' : d.name}</b><div className="text-xs text-ink-400">{d.when}</div></div>
        ...
        {tab === 'cost' && (
          <div className="space-y-2">
            ...
            {c.costs.length === 0 && <p className="text-ink-400 text-sm text-center py-6">Chưa phát sinh chi phí.</p>}
            {c.costs.map((x, i) => (
              <div key={i} className="flex items-center gap-3 bg-white rounded-2xl p-3 ring-1 ring-slate-900/5 shadow-soft">
                {x.receipt ? (
