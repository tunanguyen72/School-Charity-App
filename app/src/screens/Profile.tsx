...
import { Bell, ScrollText, FileText, Settings, LifeBuoy, LogOut, ChevronRight } from 'lucide-react'
...
const menu = [
  { icon: ScrollText, label: 'Lịch sử quyên góp' },
  { icon: FileText, label: 'Biên nhận điện tử' },
  { icon: Settings, label: 'Cài đặt' },
  { icon: LifeBuoy, label: 'Trung tâm hỗ trợ' },
]
    ...
    <div className="min-h-full bg-slate-50 pb-4">
      ...
      <div className="mx-4 bg-white rounded-2xl overflow-hidden">
        {menu.map((m, i) => {
          const Icon = m.icon
          return (
            <button key={i} className="w-full flex items-center gap-3 px-4 py-3.5 border-b border-slate-100 last:border-0 active:bg-slate-50">
