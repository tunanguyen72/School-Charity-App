...
function Kpi({ icon, label, value, tone }: { icon: ReactNode; label: string; value: string; tone: string }) {
  return (
    <Card className="!p-3.5">
      ...
      <div className="text-[17px] font-extrabold text-ink-900 leading-none">{value}</div>
...
export default function Stats() {
  ...
  if (loading) return <div className="min-h-[100svh] app-canvas"><TopBar title="Báo cáo minh bạch" back /><Loading /></div>
  if (error || !data) return <div className="min-h-[100svh] app-canvas"><TopBar title="Báo cáo minh bạch" back /><ErrorBox message={error ?? 'Lỗi'} /></div>
  ...
  return (
    <div className="min-h-[100svh] app-canvas pb-6">
      ...
      <div className="grid grid-cols-2 gap-3 px-4 mt-4">
        ...
        <Kpi icon={<Receipt className="w-4 h-4 text-ink-600" />} tone="bg-slate-100" label="Chiến dịch" value={`${o.campaignsActive}+${o.campaignsDone}`} />
      </div>

      {/* Biểu đồ theo tháng */}
      <Card className="mx-4 mt-4">
        <div className="flex items-center justify-between mb-2">
          <h2 className="font-extrabold text-ink-900">Quyên góp theo tháng</h2>
          <span className="text-[12px] text-ink-400">6 tháng gần nhất · {vndShort(monthTotal)}</span>
        </div>
      ...
      {/* Phân bổ quỹ */}
      <Card className="mx-4 mt-4">
        <h2 className="font-extrabold text-ink-900 mb-3">Phân bổ quỹ theo chiến dịch</h2>
        <DonutChart
      ...
      {/* Thu - chi */}
      <Card className="mx-4 mt-4">
        <h2 className="font-extrabold text-ink-900 mb-3">Dòng tiền: Thu — Chi</h2>
        <FlowBar raised={o.totalRaised} disbursed={o.totalDisbursed} />
      </Card>

      {/* Tiến độ chiến dịch */}
      <Card className="mx-4 mt-4">
        <h2 className="font-extrabold text-ink-900 mb-3">Tiến độ từng chiến dịch</h2>
        <div className="space-y-3">
          {data.byCampaign.map((c, i) => (
            <div key={c.slug}>
              <div className="flex items-center justify-between text-[13px] mb-1">
                <span className="flex items-center gap-1.5 text-ink-700 font-semibold truncate"><span>{c.emoji}</span>{c.title}</span>
                <span className="text-ink-400 shrink-0">{pct(c.raised, c.goal)}%</span>
              </div>
      ...
      {/* Top nhà hảo tâm */}
      <Card className="mx-4 mt-4">
        <h2 className="font-extrabold text-ink-900 mb-3">Top nhà hảo tâm 🏆</h2>
        <div className="space-y-2">
          {data.topDonors.map((d, i) => (
            <div key={i} className="flex items-center gap-3">
              <span className={`w-7 h-7 rounded-full grid place-items-center text-xs font-extrabold ${i === 0 ? 'bg-amber-100 text-amber-700' : i === 1 ? 'bg-slate-200 text-ink-600' : i === 2 ? 'bg-orange-100 text-orange-700' : 'bg-slate-100 text-ink-500'}`}>{i + 1}</span>
              <div className="flex-1 min-w-0"><b className="text-ink-900 text-sm truncate block">{d.name}</b><span className="text-[11px] text-ink-400">{d.count} lượt</span></div>
      ...
      <p className="text-center text-[11px] text-ink-400 mt-5 px-8">Số liệu được tổng hợp tự động từ giao dịch & chứng từ đã xác minh trong hệ thống.</p>
    </div>
  )
}
