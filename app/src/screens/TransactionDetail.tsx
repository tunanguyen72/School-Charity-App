...
export default function TransactionDetail() {
  ...
  if (loading) return <div className="min-h-[100svh] app-canvas"><TopBar title="Chi tiết giao dịch" back /><Loading /></div>
  if (error || !t) return <div className="min-h-[100svh] app-canvas"><TopBar title="Chi tiết giao dịch" back /><ErrorBox message={error ?? 'Không tìm thấy giao dịch'} /></div>
  ...
  return (
    <div className="min-h-[100svh] app-canvas">
      ...
      <div className="text-center pt-7">
        ...
        <div className="text-ink-400 text-sm mt-2">Giao dịch {t.status === 'verified' ? 'đã xác minh' : 'thành công'}</div>
      ...
      <div className="mx-5 mt-6 bg-white rounded-3xl p-5 text-sm">
        {[['Mã giao dịch', t.code], ['Thời gian', dateStr], ['Loại', t.kind === 'donation' ? 'Quyên góp' : t.kind === 'expense' ? 'Giải ngân' : 'Hiện vật'], [isCost ? 'Người nhận' : 'Người gửi', t.who]].map(([k, v], i) => (
          <div key={i} className="flex justify-between py-2.5 border-b border-dashed border-slate-100"><span className="text-ink-500">{k}</span><b className="text-ink-900 text-right">{v}</b></div>
        ))}
      ...
      <h3 className="font-extrabold text-ink-900 px-5 mt-5 mb-2">Minh chứng giao dịch</h3>
      <div className="mx-5 rounded-2xl border border-slate-200 bg-white p-4">
        <div className="flex items-center gap-2 font-bold text-ink-700 mb-2"><Landmark className="w-4 h-4 text-brand-600" /> BANK — CHUYỂN TIỀN THÀNH CÔNG</div>
        {[['Số tiền', t.kind === 'inkind' ? '—' : vnd(Math.abs(amt))], ['Thời gian', dateStr], ['Đối tác', t.who], ['Mã', t.code]].map(([k, v], i) => (
          <div key={i} className="flex justify-between text-[12px] py-1 border-b border-dashed border-slate-100 last:border-0"><span className="text-ink-500">{k}</span><b>{v}</b></div>
        ))}
      </div>

      <div className="mx-5 my-5 text-[12px] text-ink-500 bg-slate-100 rounded-xl px-3 py-3 flex items-start gap-2">
