...
const roleTone: Record<string, string> = { donor: 'bg-slate-100 text-ink-600', tnv: 'bg-amber-100 text-amber-700', admin: 'bg-brand-100 text-brand-700' }
...
export default function Users() {
  ...
  return (
    <div className="min-h-[100svh] app-canvas pb-6">
          ...
          <Card key={u.id} className="!p-3.5">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="font-bold text-ink-900 text-sm truncate">{u.fullName}</div>
            ...
            <div className="flex items-center gap-2 mt-3">
              <span className="text-[11px] text-ink-400 flex-1">Đã góp {vndShort(u.donated)} · {u.donationCount} lượt</span>
      ...
      <p className="text-center text-[11px] text-ink-400 mt-4 px-8">Đổi vai trò để cấp quyền Tình nguyện viên (ghi nhận hiện vật) hoặc Admin (toàn quyền).</p>
    </div>
  )
}
