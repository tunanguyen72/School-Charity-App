    ...
    <div className="min-h-[100svh] bg-slate-50 pb-6">
      ...
      {data && data.length > 0 && (
        <div className="grid grid-cols-2 gap-3 px-4 pt-4">
          <Card className="!p-3 text-center"><div className="font-extrabold text-base text-brand-700">{data.length}</div><div className="text-[10px] text-slate-400">LẦN PHÂN PHỐI</div></Card>
          <Card className="!p-3 text-center"><div className="font-extrabold text-base text-emerald-600">{totalItems}</div><div className="text-[10px] text-slate-400">ĐƠN VỊ ĐÃ TRAO</div></Card>
        </div>
      )}

      {loading && <Loading />}
      {error && <ErrorBox message={error} />}
      {data?.length === 0 && <p className="text-center text-slate-400 text-sm py-10">Chưa có lần phân phối nào.</p>}
          ...
          <Card key={d.id} className="!p-3">
            <div className="flex gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="font-bold text-slate-800 text-sm">{emojiFor(d.category)} {d.quantity} {d.unit} {d.item}</div>
                <div className="text-[12px] text-slate-500 flex items-center gap-1 mt-0.5"><MapPin className="w-3 h-3" /> {d.to} · {d.province}</div>
                <div className="text-[11px] text-slate-400 mt-0.5 truncate">{d.campaign}</div>
                {d.note && <div className="text-[11px] text-slate-400 mt-0.5 italic truncate">“{d.note}”</div>}
                <div className="text-[11px] text-slate-400 mt-1">👤 {d.by} · {new Date(d.at).toLocaleDateString('vi-VN')}</div>
              </div>
            </div>
          </Card>
        ))}
