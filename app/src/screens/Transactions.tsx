  ...
  return (
    <div className="min-h-full bg-slate-50">
      ...
      <Card className="mx-4 mt-4">
        <div className="text-sm text-slate-400">Tổng thu quỹ</div>
        ...
        <div className="flex justify-between text-[12px] text-slate-400 mt-1.5">
      ...
      <div className="flex gap-2 px-4 mt-3 overflow-x-auto no-scrollbar">
        {filters.map((x) => (
          <button key={x.key} onClick={() => setF(x.key)} className={`shrink-0 px-4 py-2 rounded-full text-[13px] font-bold border transition ${f === x.key ? 'bg-brand-600 text-white border-brand-600' : 'bg-white text-slate-500 border-slate-200'}`}>{x.label}</button>
        ))}
      </div>
        ...
        {list.map((t) => {
          ...
          return (
            <Card key={t.code} className="!p-3 flex items-center gap-3" onClick={() => nav(`/transaction/${t.code}`)}>
              ...
              <div className="flex-1 min-w-0"><b className="text-slate-800">{t.who}</b><div className="text-xs text-slate-400 truncate">{t.code}</div></div>
              <div className="text-right">
                {t.kind === 'inkind'
                  ? <b className="text-slate-700 text-[13px]">{t.label}</b>
