...
import { Bell, TrendingUp, ChevronRight, HeartHandshake, PackageCheck } from 'lucide-react'
  ...
  return (
    <div className="min-h-full bg-slate-50 pb-4">
      ...
      <div className="mx-4 rounded-3xl p-5 text-white bg-gradient-to-br from-brand-500 to-brand-800 shadow-xl shadow-brand-700/30 relative overflow-hidden">
        ...
        <div className="text-[34px] font-extrabold tracking-tight leading-none mt-1 relative">{vnd(Number(summary?.totalRaised ?? 0))}</div>
        <div className="flex items-center gap-2 mt-1.5 text-[13px] text-white/85 relative">
          + {summary?.inkindSets ?? 0} bộ hiện vật
          <span className="inline-flex items-center gap-0.5 bg-white/20 px-1.5 py-0.5 rounded-md text-[11px] font-bold"><TrendingUp className="w-3 h-3" /> 12%</span>
        </div>
        <div className="grid grid-cols-3 gap-2 mt-4 bg-white/10 rounded-2xl p-3 relative">
          {[[`${summary?.inkindSets ?? 0} bộ`, 'HIỆN VẬT'], [`${summary?.projects ?? 0}`, 'DỰ ÁN'], ['850', 'TRẺ HỖ TRỢ']].map(([a, b]) => (
      ...
      <div className="mx-4 space-y-2">
        <Card className="!p-3 flex items-center gap-3">
          ...
          <div className="flex-1 text-sm"><b>Nguyễn Văn A</b> quyên góp {vndShort(500_000)}<div className="text-xs text-slate-400">2 phút trước</div></div>
        </Card>
        <Card className="!p-3 flex items-center gap-3">
